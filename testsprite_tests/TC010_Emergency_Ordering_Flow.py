...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Emergency Order Successfully Placed').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Emergency reorder mode did not allow quick placement of an emergency order with minimal inputs and confirmation was not received.')
