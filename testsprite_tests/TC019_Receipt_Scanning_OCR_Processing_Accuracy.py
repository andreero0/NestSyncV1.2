...
async def run_test():
    ...
    try:
        ...
        # -> Input premium user email and password, then click Sign In button.
        frame = context.pages[-1]
        # Input premium user email address
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div/div/div/div/div/div/div/div/div[2]/div/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('premiumuser@example.com')
        

        frame = context.pages[-1]
        # Input premium user password
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=OCR Scan Successful: All items processed').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: OCR scanning feature did not process receipt images correctly or inventory was not updated as expected for premium users.')
