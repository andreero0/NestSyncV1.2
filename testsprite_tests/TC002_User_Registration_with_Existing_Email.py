...
async def run_test():
    ...
    try:
        ...
        # -> Fill in the registration form with an already registered email and valid password, then submit the form.
        frame = context.pages[-1]
        # Input first name
        ...
        frame = context.pages[-1]
        # Input already registered email address
        ...
        # -> Select a province from the dropdown to continue filling the form.
        frame = context.pages[-1]
        # Select Alberta as province
        elem = frame.locator('xpath=html/body/div[5]/div/div[2]/div/div/div/div/div/div/div/div/div/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input valid password and confirm password, accept Privacy Policy and Terms of Service, then submit the registration form.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError('Test case failed: Registration should fail when using an already registered email, but the success message was not found.')
