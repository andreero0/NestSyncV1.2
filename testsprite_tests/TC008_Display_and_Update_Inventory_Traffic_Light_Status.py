...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In to navigate to inventory management screen.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Inventory Updated Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The diaper inventory list did not update dynamically with correct real-time stock levels and traffic light indicators as required by the test plan.')
