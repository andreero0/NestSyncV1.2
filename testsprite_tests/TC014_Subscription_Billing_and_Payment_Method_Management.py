...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In to access the application
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Payment Method Added Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The test plan execution failed to verify adding, updating, and removing payment methods, viewing billing history, and processing Stripe payments correctly.")
