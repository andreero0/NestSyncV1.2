...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In button to proceed.
        ...
        frame = context.pages[-1]
        # Click Sign In button to login and proceed to onboarding
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Child Profile Successfully Created').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test failed: Child profile creation during onboarding did not complete successfully as required by the test plan.')
