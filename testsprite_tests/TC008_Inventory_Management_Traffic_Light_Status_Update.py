...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Inventory Fully Stocked and No Alerts').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Diaper inventory traffic light did not update correctly based on stock levels as per the test plan.")
