...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In to access privacy consent settings.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Privacy Consent Successfully Updated').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Privacy consent toggling, legal modal display, or consent logging did not occur as expected according to the test plan.')
