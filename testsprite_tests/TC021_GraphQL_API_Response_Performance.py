...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=GraphQL Query Performance Exceeded').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Core GraphQL queries and mutations did not meet the expected performance targets under normal load conditions.')
