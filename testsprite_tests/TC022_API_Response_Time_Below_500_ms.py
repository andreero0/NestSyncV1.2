...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=GraphQL API response time exceeded 500 milliseconds').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Backend GraphQL APIs did not respond within 500 milliseconds for typical requests including authentication, inventory, and analytics queries.')
