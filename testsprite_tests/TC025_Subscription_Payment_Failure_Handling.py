...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In to access user account for subscription testing.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Subscription payment succeeded').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The subscription payment failure was not handled gracefully as expected. The user was not notified with retry options after the payment failure.')
