...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Profile switch successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Unable to verify that user can switch between multiple child profiles and data updates accordingly in the UI as the expected profile switch confirmation text was not found.')
