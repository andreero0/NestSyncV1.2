...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Notification delivery confirmed via fax').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Notifications were not sent correctly via push and email only, and SMS was disabled as per user preferences. Also, notifications during quiet hours were not handled as expected.')
