...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In to access inventory management.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Inventory successfully synchronized across devices').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Unable to verify that users can quickly add and remove diaper inventory items and that UI and backend stay in sync as per the test plan.')
