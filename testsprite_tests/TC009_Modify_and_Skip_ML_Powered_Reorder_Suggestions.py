...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In to proceed.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Order quantities have been successfully updated').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: ML reorder suggestions were not displayed or user modifications to suggested order quantities were not respected as required by the test plan.')
