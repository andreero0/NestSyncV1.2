...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password to sign in and activate trial subscription.
        frame = context.pages[-1]
        # Input email address for new user to activate trial subscription
        ...
        frame = context.pages[-1]
        # Click Sign In button to log in and activate trial subscription
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Subscription Activated Successfully').first).to_be_visible(timeout=5000)
        except AssertionError:
            raise AssertionError('Test case failed: The trial activation and subscription upgrade process did not complete successfully with Canadian tax calculations as expected.')
