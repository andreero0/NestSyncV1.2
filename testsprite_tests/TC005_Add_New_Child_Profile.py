...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Child Profile Successfully Added').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Unable to add a new child profile with all required fields. The new child profile was not found in the profile list after submission.")
