...
async def run_test():
    ...
    try:
        ...
        # -> Perform login to proceed to home screen and measure time to full home screen render.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=App startup time exceeded 3 seconds').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The app startup time from launch to full home screen render exceeded the 3 seconds limit as per the test plan.')
