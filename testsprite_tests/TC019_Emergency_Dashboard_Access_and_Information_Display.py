...
async def run_test():
    ...
    try:
        ...
        # -> Retry login or check connectivity to access main navigation for emergency dashboard.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Emergency Dashboard Access Granted').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Unable to access emergency dashboard quickly and display correct medical info and emergency contacts for selected child as per the test plan.')
