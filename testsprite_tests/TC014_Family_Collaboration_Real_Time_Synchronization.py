...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Click Sign In button for User A login
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Inventory update failed due to permission error').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Real-time inventory updates and notifications did not sync correctly across multiple caregiver devices, or role-based permission enforcement failed as unauthorized actions were not properly blocked.")
