...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password to sign in and trigger the consent modal.
        ...
        frame = context.pages[-1]
        # Click Sign In button to sign in and trigger consent modal
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Data Consent Granted Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: User consent for data collection was not requested just-in-time, stored securely with audit trails, or could not be updated or withdrawn as per the test plan.")
